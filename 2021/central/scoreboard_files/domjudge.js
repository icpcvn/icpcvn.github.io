...
function checkUploadFormPublic()
{
	var problemtxt = document.getElementsByName("codename")[0].value;
	var langelt = document.getElementById("langid");
	var language = langelt.options[langelt.selectedIndex].value;
	var languagetxt = langelt.options[langelt.selectedIndex].text;
	var fileelt = document.getElementById("maincode");
	var filename = fileelt.value;
	var auxfiles = document.getElementsByName("code[]");
	var codeelt = document.getElementById("textarea_1");
	var content = editAreaLoader.getValue("textarea_1");
	...
	if ( error ) {
		...
		var question = 'Are you sure want to submit problem ' + problemtxt + "?";
		return confirm (question);
	}

}

function checkUploadFormContest()
{
	var probelt = document.getElementById("probid");
	...
	if ( error ) {
		...
		var question = 'Are you sure want to submit problem ' + problem + "?";
		return confirm (question);
	}
...
function countdown(element, time)
{
	...
	var interval = setInterval(function() {
		if (time == -1) {
			...
			if (element.startsWith('countdown-upcoming')) {
				alert('Contest has started!');
			}
			else if (element.startsWith('countdown-running')) {
				alert('Contest has finished!');	
			}
            return;
		}
		if (time < 86400) {
			...
			el.innerHTML = (hour < 10 ? '0' : '') + hour + ':' + (min < 10 ? '0' : '') + min + ':' + (sec < 10 ? '0' : '') + sec;
		}
		else {
			var day = parseInt(time / 86400);
			el.innerHTML = day + ' day' + (day == 1 ? '' : 's');
		}
        time--;
    }, 1000);
}
...
function chooseParticipant() {
	var x = document.getElementById("teamId").value;
	var rows = $('table.contest-registration tr.teams.team-members td div');
	if (x == '-1') {
		$('table.contest-registration tr').filter('.teams.team-members').hide();
	}
	else {
		$('table.contest-registration tr').filter('.teams.team-members').show();
		rows.filter('.team-members.team-'+x+'-members').show();	
		rows.not('.team-members.team-'+x+'-members').hide();	
	}
}