// Author: RR
// Fermat point O(1)
// - If at any vertex > 120 degree --> answer is that vertex
// - Otherwise, answer = P such that APB = BPC = CPA = 120.
...
bool good(Point A, Point B, Point C) {
    return (RAD_to_DEG(angle(B, A, C)) >= 120);
}

Point equi(Point A, Point B, Point C) {
    Point M = (A + B) * 0.5;
    Point dir = (B - A).rotate(PI / 2.0);

    double AB = (B - A).len();
    double h = AB * sqrt(3.0) / 2;

    Point P = M + dir * (h / dir.len());

    Line l(A, B);
    if (l.f(C) * l.f(P) >= 0) return M + M - P;
    else return P;
}
...
int32_t main() {
    ...
    if (good(A, B, C)) cout << A << endl;
    else if (good(B, A, C)) cout << B << endl;
    else if (good(C, A, B)) cout << C << endl;
    else {
        Point CC = equi(A, B, C);
        Point BB = equi(A, C, B);
